...
void DrawUtils::drawArrow(Vec2 pos, Mc_Color col, float lineWidth, float arrowSize, bool isUpArrow) {
...
void DrawUtils::drawBetterESP(Entity* ent, float lineWidth) {
	...
	AABB render(ent->getRenderPositionComponent()->renderPos, ent->getAABBShapeComponent()->aabb.width, ent->getAABBShapeComponent()->aabb.height, ent->getRenderPositionComponent()->renderPos.y - ent->getAABBShapeComponent()->aabb.lower.y);
	render.upper.y += 0.1f;
...
void DrawUtils::drawZephyr(Entity* ent, float lineWidth) {
	...
	corners[0] = Vec3(ent->getRenderPositionComponent()->renderPos.x - ent->getAABBShapeComponent()->aabb.width / 1.5f * -sin(ofs), ent->getAABBShapeComponent()->aabb.upper.y + (float)0.1, ent->getRenderPositionComponent()->renderPos.z - ent->getAABBShapeComponent()->aabb.width / 1.5f * cos(ofs));
	corners[1] = Vec3(ent->getRenderPositionComponent()->renderPos.x + ent->getAABBShapeComponent()->aabb.width / 1.5f * -sin(ofs), ent->getAABBShapeComponent()->aabb.upper.y + (float)0.1, ent->getRenderPositionComponent()->renderPos.z + ent->getAABBShapeComponent()->aabb.width / 1.5f * cos(ofs));
	corners[2] = Vec3(ent->getRenderPositionComponent()->renderPos.x - ent->getAABBShapeComponent()->aabb.width / 1.5f * -sin(ofs), ent->getAABBShapeComponent()->aabb.lower.y, ent->getRenderPositionComponent()->renderPos.z - ent->getAABBShapeComponent()->aabb.width / 1.5f * cos(ofs));
	corners[3] = Vec3(ent->getRenderPositionComponent()->renderPos.x + ent->getAABBShapeComponent()->aabb.width / 1.5f * -sin(ofs), ent->getAABBShapeComponent()->aabb.lower.y, ent->getRenderPositionComponent()->renderPos.z + ent->getAABBShapeComponent()->aabb.width / 1.5f * cos(ofs));
