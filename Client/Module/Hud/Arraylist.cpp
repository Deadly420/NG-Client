...
Arraylist::Arraylist() : Module(0x0, Category::HUD, "Displays the arraylist") {
	...
	registerFloatSetting("Opacity", &opacity, opacity, 0.f, 1.f, "Opacity: Adjust transparency from invisible (0) to solid (1)");
...
void Arraylist::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	windowSize.x = windowSize.x - padding;
	//int arrayListX = X;
	int yOffset = padding;
	if (mode.selected == 0) {
		yOffset = padding + 1;
	...
	if (moduleMgr->isInitialized() && !clickGUI->isEnabled()) {
		...
		for (std::set<ModuleContainer>::iterator mod = modContainerList.begin(); mod != modContainerList.end(); ++mod) {
			...
			underline = Vec4(windowSize.x - (textWidth + 4.f + (textPadding * 2.f)), leftRect.w, windowSize.x, leftRect.w + 1.f);
		}
