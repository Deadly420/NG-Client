...
void Compass::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	for (int off = -range; off <= range; off++) {
		...
		switch (oDeg) {
		case 0:
			drawCenteredText(Vector2(xOff, 30.f), "N", 1, majorOpacity);
			break;
		case 45:
			drawCenteredText(Vector2(xOff, 30.f), "NE", 1, majorOpacity);
			break;
		case 90:
			drawCenteredText(Vector2(xOff, 30.f), "E", 1, majorOpacity);
			break;
		case 135:
			drawCenteredText(Vector2(xOff, 30.f), "SE", 1, majorOpacity);
			break;
		case 180:
			drawCenteredText(Vector2(xOff, 30.f), "S", 1, majorOpacity);
			break;
		case 225:
			drawCenteredText(Vector2(xOff, 30.f), "SW", 1, majorOpacity);
			break;
		case 270:
			drawCenteredText(Vector2(xOff, 30.f), "W", 1, majorOpacity);
			break;
		case 315:
			drawCenteredText(Vector2(xOff, 30.f), "NW", 1, majorOpacity);
			break;
		}
		...
		for (auto it = result.first; it != result.second; it++) {
			...
			Vector2 pos = Vector2(xOff, 31.f);

			int overlapping = 0;
			const float tSize = 0.75f;
			const float tWidth = DrawUtils::getTextWidth(&pName, tSize);
			pos.x -= tWidth / 2;
			const Vector2 myTextRange = Vector2(pos.x, pos.x + tWidth); 
			for (const Vector2 otherTextRange : stacking) {
		...
		if (off != 0 && minorOpacity > 0) {
			if ((oDeg % 15) != 0) {
				if ((oDeg % 5) == 0) {
					DrawUtils::fillRectangle(Vector4(xOff - 0.25f, 16, xOff + 0.25f, 19), Mc_Color(255, 255, 255), minorOpacity);
				}
				continue;
			}

			// Bigger line with degree displayed
			DrawUtils::fillRectangle(Vector4(xOff - 0.5f, 15, xOff + 0.5f, 20), Mc_Color(255, 255, 255), minorOpacity);
			drawCenteredText(Vector2(xOff, 20.f), std::to_string(oDeg), 0.75f, minorOpacity);
		}
	}

	// Center line
	DrawUtils::fillRectangle(Vector4(sCenter - 0.5f, 15, sCenter + 0.5f, 25), Mc_Color(255, 255, 255), opacity);
	drawCenteredText(Vector2(sCenter, 25.f), std::to_string(deg), 0.75f, opacity);
	DrawUtils::flush();
}

void Compass::drawCenteredText(Vector2 pos, std::string text, float size, float textOpacity) {
