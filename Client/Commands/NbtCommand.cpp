...
NbtCommand::NbtCommand() : Command("nbt", "read and write NBT tags to/from your clipboard (You have to point at an entity/block entity)", "<read/write>") {
...
bool NbtCommand::execute(std::vector<std::string>* args) {
	...
	if (args->at(1) == "read" || args->at(1) == "save") {
		...
		if (args->at(1) == "save" && item != nullptr) {
			...
			if (level->getEntity() != nullptr) {
				if (Game.getRakNetConnector()->isonaServer()) {
					clientMessage("%sNBT tags for mobs only works in local world!", RED);
					return true;
				}
				...
				clientMessage("%sCouldn't find NBT tags!", RED);
				return true;
			}
		}
		...
		clientMessage("%s%s", GREEN, "CompoundTag copied:");
		clientMessage(builtStr.c_str());
		...
		if (tag.size() > 1 && tag.front() == MojangsonToken::COMPOUND_START.getSymbol() && tag.back() == MojangsonToken::COMPOUND_END.getSymbol()) {
			...
			clientMessage("%sInvalid NBT tag!", RED);
			return true;
		}
		...
		clientMessage("%s%s", GREEN, "Successfully loaded mojangson !");
	} else {
		clientMessage("%s%s", RED, "Couldn't execute command correctly");
	}
