...
EnchantCommand::EnchantCommand() : Command("enchant", "Enchants items", "<enchantment> [level] <mode: auto / manual : 1/0>") {
...
bool EnchantCommand::execute(std::vector<std::string>* args) {
	...
	if (args->at(1) != "all") {
		try {
			...
			clientMessage("Exception while trying to get enchant string");
			enchantId = assertInt(args->at(1));
		}
	}
	...
	if (item == nullptr || item->item == nullptr) {
		clientMessage("Please hold an item!");
		return false;
	}
	...
	if (strcmp(args->at(1).c_str(), "all") == 0) {
		...
		clientMessage("%sEnchant successful!", GREEN);
	} else {
		void* enchantData = malloc(0x60);
		if (enchantData != nullptr) {
			memset(enchantData, 0x0, 0x60);
		...
		if (addEnchant(enchantData, enchantPair)) {  // Upper 4 bytes = level, lower 4 bytes = enchant type
			...
			clientMessage("%sEnchant failed, try using a lower enchant level", RED);
		}
