...
PathCommand::PathCommand() : Command("path", "Joe path", "<y|xz|xyz> [args]"){
	registerAlias("joe");
}
...
bool PathCommand::execute(std::vector<std::string>* args) {
	...
	if (mod->isEnabled()) {
		clientMessage("Joe is already enabled, disable Joe to use this command");
		return true;
	}
	...
	if (cmd == "p" || cmd == "player") {
		...
		if (pos.iszero()) {
			clientMessage("%s Player \"%s\" could not be found!", GOLD, nameOfPlayer.c_str());
			return true;
		}
