...
XpCommand::XpCommand() : Command("xp", "Add experience to your player", "xp <amount>L") {
}
...
bool XpCommand::execute(std::vector<std::string>* args) {
	...
	if (std::all_of(experienceAmount.begin(), experienceAmount.end(), ::isdigit)) {
		...
		clientMessage("%sAdded %i experience %s", GREEN, experience, isAddingLevels ? "levels" : "points");
	} else {
		clientMessage("%sInvalid amount of experience!", RED);
	}
