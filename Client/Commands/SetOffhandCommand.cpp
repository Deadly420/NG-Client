...
SetOffhandCommand::SetOffhandCommand() : Command("setoffhand", "Spawn items in ur offhand", "<itemName> <count> <itemData>") {
	registerAlias("soh");
}
...
bool SetOffhandCommand::execute(std::vector<std::string>* args) {
	...
	clientMessage("%sSuccessfully set item to offhand!", GREEN);
	return true;
}
