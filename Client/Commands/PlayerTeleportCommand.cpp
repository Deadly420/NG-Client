...
PlayerTeleportCommand::PlayerTeleportCommand() : Command("playertp", "Teleports to players coordinates", "<NameOfThePlayer>") {
}
...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessage("%s[%sNG%s] %sCouldn't find player: %s!", BOLD, AQUA, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}

	Game.getLocalPlayer()->setPos(pos);
	clientMessage("%s[%sNG%s] %sTeleported to %s", BOLD, AQUA, WHITE, GREEN, playerName.c_str());
	return true;
}
