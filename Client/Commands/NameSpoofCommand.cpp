...
NameSpoofCommand::NameSpoofCommand() : Command("namespoof", "Spoof your name on realms/bds", "<name/reset> <name>") {
	registerAlias("ns");
...
bool NameSpoofCommand::execute(std::vector<std::string>* args) {
	if (args->at(1) == "name" && args->size() > 2) {
		...
		clientMessage("%s[%sNG%s] %sSet fakename to %s%s%s, please reconnect!", BOLD, AQUA, WHITE, GREEN, GRAY, name->getText(), GREEN);
		return true;
	} else if (args->at(1) == "reset") {
		Game.setFakeName(NULL);
		clientMessage("%s[%sNG%s] %sReset fakename!", BOLD, AQUA, WHITE, GREEN);
		return true;
	}
	return false;
}
