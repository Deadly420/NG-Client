...
public:
	...
	Vector3i* getPairedPos() {
		return reinterpret_cast<Vector3i*>(reinterpret_cast<__int64>(this) + 0x228);
	}
	...
	AABB getFullAABB() {
		if (!isPaired()) {
			if (isBarrelBlock() || isShulkerBlock())
				return AABB(this->posI.toVector3t(), this->posI.toVector3t().add(1));
			return AABB(this->posI.toVector3t().add(0.0625, 0, 0.0625), this->posI.toVector3t().add(1 - 0.0625, 1 - 1.f / 8, 1 - 0.0625));
		}
		Vector3i first = this->posI;
		Vector3i second = *getPairedPos();

		if (first.x > second.x || first.z > second.z)
			std::swap(first, second);

		return AABB(first.toVector3t().add(0.0625, 0, 0.0625), second.toVector3t().add(1 - 0.0625, 1 - 1.f / 8, 1 - 0.0625));
	}
